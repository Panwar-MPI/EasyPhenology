...
def integral_smoothing(df):
    ...
    # Create empty list for year and Var_smooth
    Year = []
    Var_smooth=[]
    ...
    for j in range(len(Var_Years)):  # Loop for each year
        ...
        if df_jyear['Var'].isnull().sum()<50: 
            ...
            #For smoothing add 10 days before and after the year from the same year end points
            if j>0:
...
def EasyPhenology(df, Threshold_value):
    ...
    for j in range(len(Var_Years)): #Loop over years
